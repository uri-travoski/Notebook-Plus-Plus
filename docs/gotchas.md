...
## SSR data loading + PWA (Phase 11)
- **Never raw-`$fetch` an authed `/api` route at SSR setup top-level** — `$fetch` does not forward the incoming request's session cookie, so it 401s and errors the whole page on a hard load. Use **`useFetch`** (forwards cookies via `useRequestFetch`) or fetch in **`onMounted`** (client, browser sends the cookie). This bit `pages/settings/index.vue` + `components/AiKeysManager.vue`.
- **`@vite-pwa/nuxt`** registers/activates the SW automatically but did NOT inject `<link rel="manifest">` into our SSR HTML — add it explicitly in `nuxt.config app.head.link` or the app isn't installable. Icons are generated from `public/icon.svg` via ImageMagick (`magick`).
